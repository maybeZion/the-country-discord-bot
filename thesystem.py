...
async def on_member_join(member):
    await member.send("Welcome to The Country, {}. ".format(member.name))
...
class Economy(commands.Cog):
    ...
    async def work(self, ctx):
        ...
        for item in self.jobs:
            if ctx.message.author in item.currentworkers:
                worked_job = item
                y = worked_job.jobpay() * item.tax
        if worked_job is not None:
            ...
            if x:
                try:
                    self.ledger[str(ctx.message.author.id)] += y
                except KeyError:
                    self.ledger[str(ctx.message.author.id)] = y
                save(worked_job, worked_job.title)
                await ctx.send('You made **{} credits** working!'.format(y))
            else:
    ...
    @edit.command()
    async def tax(self, ctx, job, tax):
        if self.temp_leader_bool:
            for job in self.jobs:
                if job.title == job and tax < .5:
                    self.tax = tax
                    save(job, job.title)
        else:
            await ctx.send("Only the current leader can use this command.")
    ...
    async def apply(self, ctx, applied_job):
        ...
        for item in ctx.message.author.roles:
            ...
            elif item.name == 'Vagrant':
                citizentype_temp = item
            elif item.name == 'Cabinet':
                citizentype_temp = item
        for item in self.jobs:
        ...
        if type(applied_job) == str:
            ...
            await ctx.send('Congratulations, you have a job! You can now do `./job work` to work once per hour.')
        else:
...
class Party(commands.Cog):
    ...
    async def join(self, ctx, party : str):
        ...
        for item in ctx.message.author.roles:
            if 'Party' in item.name:
                await ctx.send('You are already in a party. Do `./party leave` before running this command.')
                x = True
                break
        if x is None:
            if party in 'redyellowbluegreen':
                self.pending[ctx.message.author.mention] = party
    ...
    async def accept(self, ctx, member : discord.Member):
        ...
        if x:
            try:
                if self.pending[member.mention].capitalize() in y.name:
                    await member.add_roles(y)
                    del self.pending[member.mention]
